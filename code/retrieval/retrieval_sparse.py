...
class BM25Retrieval:
    def __init__(
        ...
        Summary:
            Passage 파일을 불러오고 BM25api를 선언하는 기능을 합니다.
        """
    ...
    def retrieve(
        ...
        if isinstance(query_or_dataset, str):
            ...
            for idx, example in enumerate(
                tqdm(query_or_dataset, desc="Sparse retrieval using BM25: ")
            ):
                tmp = {
                    # Query와 해당 id를 반환합니다.
                    "question": example["question"],
...
def run_sparse_retrieval(
    ...
    if data_args.retrieval_class == "tfidf":
    ...
    # train data 에 대해선 정답이 존재하므로 id question context answer 로 데이터셋이 구성됩니다.
        ...
        os.makedirs("./outputs/valid_dataset/", exist_ok=True)
        df.to_csv(os.path.join("./outputs/valid_dataset/", "valid_retrieval.csv"))
        f = Features(
            {
                'context': Value(dtype='string', id=None),
                'id': Value(dtype='string', id=None),
